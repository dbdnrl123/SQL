...
create table `bank_account`(
	`a_no` varchar(11) primary key not null default '000-00-0000',
...
#실습 2-10
select * from `bank_account` where `a_balance` >=1000000 and `a_item_dist` like 's1';
...
#실습 2-19
select * from `bank_transaction` where `t_amount` >=1 order by `t_dist`, `t_amount` desc;

#실습 2-20
select count(`t_dist` )as`입금 건수`, 
